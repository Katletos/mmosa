...
// }
struct Results {
    average_worker_waiting_time: u32,
    average_order_time: u32,
    average_busy_tables: u32,
    average_free_workers: u32,
    dispatched_clients: usize,
...
impl Results {
    pub fn of(sim: Simulation) -> Self {
        let average_worker_waiting_time = sim.average_order_time.iter().sum::<u32>() / sim.average_order_time.len() as u32;
        let average_order_time = sim.average_worker_waiting_time.iter().sum::<u32>() / sim.average_worker_waiting_time.len() as u32;
        let average_busy_tables = sim.average_busy_tables.iter().sum::<u32>() / sim.average_busy_tables.len() as u32;
        let average_free_workers = sim.average_free_workers.iter().sum::<u32>() / sim.average_free_workers.len() as u32;
        ...
        Self {
            ...
            dispatched_clients: sim.dispatched_clients_count,
            not_dispatched_clients: sim.not_dispatched_clients,
...
impl Simulation {
    ...
    fn process_tick(&mut self, time: u32) {
        ...
        while let Some(event) = self.events.pop_front() {
            match event {
                ...
                Event::WaitingForWorker(start_time, leave_time, is_first_time) => {
                    ...
                    if leave_time <= time {
                        self.available_tables += 1;
                        self.average_worker_waiting_time.push(leave_time - start_time);
                    ...
                    if self.available_workers > 0 {
                        ...
                        let free_worker_time = thread_rng().gen_range(1..2) + time;
                        // let free_worker_time = time;
                        new_events.push_back(Event::WorkerWalkingDance(free_worker_time));

                        self.average_worker_waiting_time.push(time - start_time);
                    } else {
                        new_events.push_back(Event::WaitingForWorker(time, leave_time, is_first_time));
                    }
                }
                ...
                Event::WorkerWalkingDance(free_worker_time) => {
                    if time >= free_worker_time {
                        ...
                        self.available_workers += 1;
                        assert!(self.available_workers <= MAX_WORKERS);
                ...
                Event::WaitingForFood(finish_food_time) => {
                    if time >= finish_food_time {
                        ...
                        let end_consume_time = thread_rng().gen_range(1..5) + time;
        ...
        self.average_busy_tables.push(MAX_TABLES - self.available_tables);
    ...
    fn generate_new_events(&mut self, tick: u32) {
        let is_client_arrived = thread_rng().gen_bool(0.55);
